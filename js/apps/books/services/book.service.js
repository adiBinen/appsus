...
function getBookById(id) {
    let book = booksDB.find(book => book.id === id)
    if (book) return Promise.resolve(book);
    else return Promise.reject('No book was found');
}

function addBookReview(review, id) {
    let book = booksDB.find(book => book.id === id);
    if (book) {
        review.id = utilService.generateId();
        book.reviews.push(review);
        storageService.saveToLocal(BOOKS_KEY, booksDB);
        return Promise.resolve('Review added successfully');
    } else return Promise.reject('Error, book was not found.');
}

function deleteBookReview(reviewId, bookId) {
    let book = booksDB.find(book => book.id === bookId);
    if (!book) return Promise.reject('Book not found or not exists');
    let filteredReviews = book.reviews.filter(review => review.id !== reviewId);
    book.reviews = filteredReviews;
    storageService.saveToLocal(BOOKS_KEY, booksDB);
    return Promise.resolve('Review was successfully deleted');
}

// Creates an object while deconstructing GoogleBook's object to fit this app's model
function addGoogleBook(googleBook) {
    // Check if book was already imported
    if (booksDB.find(book => book.title === googleBook.volumeInfo.title)) {
        return Promise.reject('Failed to add the book');
    }
    let book = {};
    ...
    book.publishedDate = googleBook.volumeInfo.publishedDate.split('-')[0];
    book.description = googleBook.volumeInfo.description;
    // Thumbnail raised error - check when setting
    let thumbnail = googleBook.volumeInfo.imageLinks.thumbnail;
    book.thumbnail = (thumbnail)? thumbnail : '';

    book.language = googleBook.volumeInfo.language;
    book.pageCount = googleBook.volumeInfo.pageCount;

    // listPrice raised errors, therefore, randomize it if doesn't exist
    let listPrice = googleBook.saleInfo.listPrice;
    if (listPrice && listPrice.amount && listPrice.currencyCode) book.listPrice = listPrice;
    else {
        book.listPrice = {};
        book.listPrice.amount = utilService.getRandomIntInclusive(50,300);
        book.listPrice.currencyCode = (utilService.getRandomIntInclusive(0,1))? 'USD' : 'EUR';
    }
    book.listPrice.isOnSale = (utilService.getRandomIntInclusive(0, 1)) ? true : false;
    book.reviews = [];
    
    booksDB.push(book);
    storageService.saveToLocal(BOOKS_KEY, booksDB);
    return Promise.resolve('Added book: ' + book.id);
}

function _createBooks() {
    let books = storageService.loadFromLocal(BOOKS_KEY);
    if (!books) {
        books = _premadeBooks();
...
// List received from JSON
function _premadeBooks() {
    return [
        {
            "id": "OXeMG8wNskc",
